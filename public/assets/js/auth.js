...
$("#companyUID").keyup((e) => {
  ...
  if (value.length > 4) {
    fetch(`/api/getLocationsByCompany/${value}`)
      .then((data) => {
        return data.json();
      })
      .then((res) => {
        if (res.length === undefined) {
          ...
          if (res.length !== undefined && res.length > 0) {
            ...
            $("#locationUID")[0].selectize.addOption({
              locationName: "Please choose a location",
              locationUID: "00",
            });
            $("#locationUID")[0].selectize.load(function (callback) {
              callback(res);
            });
            ...
            $("#locationNameHelp").text(
              "No locations found for the company Id entered, Please contact your Admin."
            );
          }
        }
      });
